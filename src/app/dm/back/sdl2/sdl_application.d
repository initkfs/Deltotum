module app.dm.back.sdl2.sdl_application;
...
import app.core.apps.app_init_ret : AppInitRet;
import app.core.utils.factories : Provider;
import app.dm.kit.apps.continuously_application : ContinuouslyApplication;
...
import app.dm.back.sdl2.sdl_event_processor : SdlEventProcessor;
import app.dm.kit.scenes.scene_manager : SceneManager;
import app.dm.kit.graphics.graphics : Graphics;
import app.dm.kit.interacts.interact : Interact;
...
import app.dm.back.sdl2.sdl_lib : SdlLib;
import app.dm.back.sdl2.img.sdl_img_lib : SdlImgLib;
import app.dm.back.sdl2.mix.sdl_mix_lib : SdlMixLib;
import app.dm.back.sdl2.ttf.sdl_ttf_lib : SdlTTFLib;
import app.dm.back.sdl2.sdl_window : SdlWindow;
import app.dm.back.sdl2.sdl_window : SdlWindowMode;
import app.dm.back.sdl2.sdl_renderer : SdlRenderer;
import app.dm.back.sdl2.sdl_joystick : SdlJoystick;
...
import app.dm.back.sdl2.sdl_texture : SdlTexture;
import app.dm.back.sdl2.sdl_surface : SdlSurface;
import app.dm.back.sdl2.ttf.sdl_ttf_font : SdlTTFFont;
import app.dm.back.sdl2.img.sdl_image : SdlImage;
...
import app.dm.com.graphics.com_image : ComImage;
import app.dm.com.platforms.com_system : ComSystem;

import app.dm.kit.windows.window : Window;

import app.dm.kit.apps.loops.integrated_loop : IntegratedLoop;
import app.dm.kit.apps.loops.interrupted_loop : InterruptedLoop;
...
import app.dm.sys.cairo.libs : CairoLib;

//import app.dm.sys.chipmunk.libs : ChipmLib;
...
class SdlApplication : ContinuouslyApplication
{
    ...
    override AppInitRet initialize(string[] args)
    {
        ...
        import app.dm.back.sdl2.sdl_keyboard : SdlKeyboard;

        auto keyboard = new SdlKeyboard;

        import app.dm.kit.inputs.clipboards.clipboard : Clipboard;
        import app.dm.back.sdl2.sdl_clipboard;
        ...
        import app.dm.back.sdl2.sdl_cursor : SDLCursor;
        ...
        try
        {
            ...
            {
                import app.dm.kit.inputs.cursors.system_cursor : SystemCursor;
        ...
        if (!cursor)
        {
            import app.dm.kit.inputs.cursors.empty_cursor : EmptyCursor;
        ...
        cairoLibForLoad.onAfterLoad = () {
            ...
            import KitConfigKeys = app.dm.kit.kit_config_keys;
        ...
        import app.dm.back.sdl2.sdl_screen : SDLScreen;
            ...
            if (mouseEvent.event == PointerEvent.Event.down)
            {
                ...
                foreach (obj; window.scenes.currentScene.activeSprites)
                {
                    if (obj.bounds.contains(mouseEvent.x, mouseEvent.y))
                    {
                        ...
                        import app.dm.kit.events.focus.focus_event : FocusEvent;
    ...
    {
        import app.dm.back.sdl2.sdl_system : SDLSystem;
        ...
        SdlWindowMode mode = SdlWindowMode.none)
    {
        ...
        {
            import app.dm.kit.assets.fonts.bitmap.bitmap_font_generator : BitmapFontGenerator;
        ...
        if (uservices.config.containsKey(KitConfigKeys.debugScene))
        {
            ...
            {
                import app.dm.gui.supports.editors.guieditor : GuiEditor;
