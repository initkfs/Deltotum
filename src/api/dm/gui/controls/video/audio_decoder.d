    ...
    override void run()
    {
        ...
        while (_running)
        {
            ...
            if(isRead != ContainerResult.success){
                ...
                logger.errorf("Error read audio packet from queue with size %s: %s, ri %s, wi: %s", packetQueue.size, isRead, packetQueue.readIndex, packetQueue.writeIndex);
            }
            ...
            int targetSamples = swr_get_out_samples(audioConvertContext, frame.nb_samples);
            ...
            foreach (i; 0..writeBytes)
            {
                const isWrote = buffer.write(audioBuff[i .. i+1]);
            }

            //const isWrite = buffer.writeSync(audioBuff[0 .. writeBytes]);

            // import std;

            // debug writefln("Write to audio buffer %s bytes: %s, cap %s, source bytes %s, full %s", writeBytes, isWrite, buffer.capacity, audioBuffSize, buffer.isFull);
