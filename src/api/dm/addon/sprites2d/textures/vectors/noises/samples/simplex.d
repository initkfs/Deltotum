module api.dm.addon.sprites2d.textures.vectors.noises.samples.simplex;

import api.dm.addon.sprites2d.textures.vectors.noises.samples.sample_noise : SampleNoise;
import api.dm.kit.graphics.styles.graphic_style : GraphicStyle;
import api.dm.kit.graphics.canvases.graphic_canvas : GraphicCanvas;
import api.dm.kit.graphics.colors.rgba : RGBA;
import api.dm.kit.graphics.colors.hsva : HSVA;
import api.dm.kit.graphics.canvases.graphic_canvas : GraphicCanvas;
import api.dm.addon.sprites2d.textures.vectors.noises.samples.permutation_table : PermutationTable;
...
class Simplex : SampleNoise
{
    ...
    override float sample1D(float x)
    {
        ...
        x = (x + offsetX) * frequency * 0.5f;

        int i0 = cast(int) Math.floor(x);
        int i1 = i0 + 1;
        float x0 = x - i0;
        float x1 = x0 - 1.0f;
    ...
    override float sample2D(float x, float y)
    {
        ...
        float xs = x + s;
        float ys = y + s;
        int i = cast(int) Math.floor(xs);
        int j = cast(int) Math.floor(ys);
    ...
    override float sample3D(float x, float y, float z)
    {
        ...
        x = (x + offsetX) * frequency * 0.5f;
        y = (y + offsetY) * frequency * 0.5f;
        z = (z + offsetZ) * frequency * 0.5f;

        // Simple skewing factors for the 3D case
        enum float F3 = 0.333333333f;
        enum float G3 = 0.166666667f;

        float n0 = 0, n1 = 0, n2 = 0, n3 = 0; // Noise contributions from the four corners
